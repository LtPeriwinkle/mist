...
impl Keybinds {
    pub fn from_raw(raw: &KeybindsRaw) -> Result<Self, String> {
        Ok(Keybinds {
            ...
            skip_split: Keycode::from_name(&raw.skip_split).ok_or("Skip split keybind could not be parsed")?,
            un_split: Keycode::from_name(&raw.un_split).ok_or("Unsplit keybind could not be parsed")?,
            ...
            load_config: Keycode::from_name(&raw.load_config).ok_or("Load config keybind could not be parsed")?,
        })
    }
}
