        ...
        match ctx {
            ...
            table::TableContext::Cell => {
                ...
                if col == 0 {
                    	...
                    	inp.set_callback2(move |input| {RUN.lock().unwrap().set_name(input.value(), row as usize);})
                } else if col == 1 {
                    	inp.set_value(&ms_to_readable(RUN.lock().unwrap().get_times()[row as usize]));
                    	inp.set_callback2(move |input| {RUN.lock().unwrap().set_time(str_to_ms(input.value()), row as usize);})
                } else if col == 2 {
			inp.set_value(&ms_to_readable(RUN.lock().unwrap().get_golds()[row as usize]));
                    	inp.set_callback2(move |input| {RUN.lock().unwrap().set_gold_time(row as usize, str_to_ms(input.value()));})
		}
                t.add(&inp);
            }
            _ => {}
